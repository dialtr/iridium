...
.set MAGIC,    0x1BADB002       /* Mmagic number searched for by bootloader. */
 ...
 * it, then alllocating 16384 bytes for it, and finally creating a symbol
 ...
 * smaller because it does not contain an uninitialized stack. The stack
 * on x86 must be 16-byte aligned according to the System V ABI standard
 * and de-facto extensions. The compiler will assume the stack is properly
 * aligned and failure to align the stack will result in undefined
 * behavior.
 */
.section .bss
.align 16
stack_bottom:
.skip 16384  # 16 KiB stack region
stack_top:
...
_start:
    ...
    * the stack (as it grows downwards on x86 systems). This is necessarily
    ...
    * C++ faetures such as global constructors and exceptions will require
    * runtime support to work as well.
    */

   /* TODO(tdial): Initialize CPU State */

   /*
    * Enter the high-level kernel. The ABI requires the stack to be 16-byte
    * aligned at the time of the call instruction (which afterwards pushes
    * the return pointer of size 4 bytes). The stack was originally 16-byte
    * aligned above, and since we've pushed a multiple of 16 bytes to the 
    * stack since (we've pushed zero bytes so far), the alignment is
    ...
    *    Since interrupts have been disabled above, this will never happen
    *    and the machine will lock up.
    *
